...
- AC-001: Given a clean database, when Flyway runs, then the `notification` table is created with the send-email columns (`notification_id`, `notification_type`, `status`, `send_to_address`, `status_code`, `error_message`, `client_context`) and non-null audit timestamps (`created_at`, `updated_at`), and no eventstore/Liquibase objects are created.
